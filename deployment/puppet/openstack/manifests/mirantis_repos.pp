...
class openstack::mirantis_repos (
  ...
  $deb_mirror   = 'http://172.18.3.14/ubuntu-repo/mirror.yandex.ru/ubuntu',
  $deb_updates  = 'http://172.18.3.14/ubuntu-repo/mirror.yandex.ru/ubuntu',
  $deb_security = 'http://172.18.3.14/ubuntu-repo/mirror.yandex.ru/ubuntu',
  $deb_fuel_folsom_repo      = 'http://172.18.3.14/ubuntu-repo/precise-fuel-folsom',
  $deb_cloud_archive_repo    = 'http://172.18.3.14/ubuntu-cloud.archive.canonical.com/ubuntu',
  $deb_rabbit_repo           = 'http://172.18.3.14/ubuntu-repo/precise-fuel-folsom',
  $enable_epel  = false,
  $fuel_mirrorlist           = 'http://download.mirantis.com/epel-fuel-folsom-2.1/mirror.internal-stage.list',
  $mirrorlist_base           = 'http://172.18.3.14/centos-repo/mirror-6.3-os.list',
  $mirrorlist_updates        = 'http://172.18.3.14/centos-repo/mirror-6.3-updates.list',
      ...
      if $type == 'custom' {
        if $enable_test_repo {
          apt::source { 'precise-fuel-folsom':
            ...
            repos       => 'main',
            key         => 'F8AF89DD',
            key_source  => 'http://172.18.3.14/ubuntu-repo/precise-fuel-folsom/Mirantis.key',
            include_src => false,
          }
        } else {
          apt::source { 'precise-fuel-folsom':
        ...
        apt::source { 'cloud-archive':
          ...
          repos       => 'main',
          key         => '5EDB1B62EC4926EA',
          key_source  => 'http://172.18.3.14/ubuntu-repo/precise-fuel-folsom/cloud-archive.key',
          # key_server   => "pgp.mit.edu",
          include_src => false,
        }
